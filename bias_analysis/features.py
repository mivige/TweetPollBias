...
from bias_analysis.config import PROCESSED_DATA_DIR, FIGURES_DIR, REPORTS_DIR
...
def pearson_correlation(save_plots: bool = typer.Option(True, help="Save diagnostic plots to files"),
    ...
    def compute_formality_bias(row):
        ...
        Returns:
            float: -1.0 (Favors Biden) to +1.0 (Favors Trump)
        """
        ...
        return float(np.sign(trump_formal - biden_formal))
    ...
    def compute_poll_outcome_bias(row):
        """
        Calculate poll outcome bias (-1=Biden win, +1=Trump win).
    ...
    def check_assumptions(df, variables_dict, save_plots=True):
        ...
        if save_plots and len(clean_df) > 10:
            ...
            if save_plots:
                plot_path = FIGURES_DIR / "assumption_check_distributions.png"
    ...
    summary_path = REPORTS_DIR / "correlation_analysis_summary.json"
    ...
    if save_plots and not correlation_df.empty:
        ...
        def create_final_correlation_heatmap(corr_results_df, markers_dict, outcomes_dict):
            ...
            # Save final heatmap
            final_plot_path = FIGURES_DIR / "final_correlation_heatmap.png"
